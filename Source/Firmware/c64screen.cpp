...
/_______  /|__\____ |\___  >__|_ \__|\___  >__|_ \     \______  /\_____  /\____   | 
 ...
 RasPiC64 - A framework for interfacing the C64 and a Raspberry Pi 3B/3B+/Pi Zero 2
          - menu code
...
const char errorMessages[NUM_ERRORMESSAGES][52] = {
...
int printFileTree( s32 cursorPos, s32 scrollPos )
{
	...
	while ( lines < DISPLAY_LINES && idx < nDirEntries ) 
	{
		...
		if ( dir[ idx ].f & DIR_KERNAL_FILE && !wireKernalAvailable )
			color = 12;
...
void printBrowserScreen()
{
	...
	for ( int i = 0; i < DISPLAY_LINES; i++ )
	{
		...
		c64color[ 38 + ( i + 3 ) * 40 ] = skinValues.SKIN_BROWSER_TEXT_FOOTER;
	}
...
//
// this is unelegant and should be changed at some point: 
// "checkForScreenFade" is essentially "handleC64" but without side-effects
//
int checkForScreenFade( int k, char *menuItemStr, u32 *startC128 = NULL )
{
	...
	{
		if ( k == VK_SHIFT_RETURN ) return 3;

		if ( k == VK_RETURN && joyIdx != -1 )
		{
		...
		int tempHasKernal = subHasKernal;
		subHasKernal = hasKernalAlready;

		if ( r == ACT_SIDKICK_CFG ) return 3;

		if ( ( subSID == 1 ) && ( ( r == ACT_SID ) || ( k == VK_RETURN ) ) ) return 3;

		if ( ( subGeoRAM == 1 ) && ( ( ( r == ACT_GEORAM ) || ( k == VK_RETURN ) ) ) ) return 3;
	...
	if ( menuScreen == MENU_BROWSER )
	{
		...
		if ( typeInName == 0 && (k == VK_RETURN || k == VK_SHIFT_RETURN) )
		{
				  ...
				  ( subSID && !wireSIDAvailable && settings[10] == 0 ) ) ) // no SID-wire, FM emulation off
			{
				...
				if ( (dir[ c ].f & DIR_FILE_IN_D64 && ((dir[ c ].f>>SHIFT_TYPE)&7) == 2) || dir[ c ].f & DIR_PRG_FILE || dir[ c ].f & DIR_CRT_FILE || dir[ c ].f & DIR_BIN_FILE || dir[ c ].f & DIR_KERNAL_FILE )
				{
					...
					extern u32 wireKernalAvailable;
					if ( dir[ curC ].f & DIR_KERNAL_FILE && wireKernalAvailable )
					{
						return 3;
					}
...
//		if ( k == 92 && joyIdx != -1 )
		...
		int temp;
		//logger->Write( "h", LogNotice, "--------------" );
		//logger->Write( "h", LogNotice, "subHasKernal=%d", subHasKernal );
		int hasKernalAlready = subHasKernal;
		subHasKernal = -1;
		
		int r = getMainMenuSelection( k, &filename, &temp, menuItemStr );
		//logger->Write( "h", LogNotice, "r=%d   subHasKernal=%d", r, subHasKernal );
		...
		if ( fileExists( logger, DRIVE, filename ) <= 0 )
		{
			hasKernalAlready = subHasKernal = -1;
...
//						logger->Write( "exec", LogNotice, "file: %s", path );


				if ( (dir[ c ].f & DIR_FILE_IN_D64 && ((dir[ c ].f>>SHIFT_TYPE)&7) == 2) || dir[ c ].f & DIR_PRG_FILE || dir[ c ].f & DIR_CRT_FILE || dir[ c ].f & DIR_BIN_FILE || dir[ c ].f & DIR_KERNAL_FILE )
				{
					logger->Write( "exec", LogNotice, "file in d64, prg, crt, or bin" );
					...
					if ( dir[ curC ].f & DIR_KERNAL_FILE && wireKernalAvailable )
					{
						logger->Write( "RaspiMenu", LogNotice, "-> launch kernal %s", path );
						strcpy( FILENAME, path );
						*launchKernel = 2;
						return;
					}
					...
					if ( dir[ curC ].f & DIR_PRG_FILE ) 
					{
						logger->Write( "exec", LogNotice, "prg" );
						if ( strstr( path, "PRG128") != 0 && modeC128 == 0 )
						{
							*launchKernel = 0;
							errorMsg = errorMessages[ 5 ];
					...
					if ( dir[ curC ].f & DIR_CRT_FILE || dir[ curC ].f & DIR_BIN_FILE ) 
					{
						logger->Write( "exec", LogNotice, "crt" );
						u32 err = 0;
					...
					if ( dir[ curC ].f & DIR_FILE_IN_D64 )
					{
						logger->Write( "exec", LogNotice, "file in d64" );
						if ( ( subSID && octaSIDMode && !wireSIDAvailable ) ||
							 ( subSID && !wireSIDAvailable && settings[10] == 0 ) ) // no SID-wire, FM emulation off
						{
							errorMsg = errorMessages[ 6 ];
				...
				{
					logger->Write( "exec", LogNotice, "want to load psid" );
	...
	if ( menuScreen == MENU_CONFIG )
	{
		...
		if ( k == VK_UP )
		{
			...
			{
				if ( settings[2] > 2 && curSettingsLine == 16 )
...
/*	char b[20];
	...
	if ( subGeoRAM && subHasKernal )
	{
		...
		//               "012345678901234567890123456789012345XXXX"
		printC64( 31, 23, "F7", skinValues.SKIN_MENU_TEXT_FOOTER, 128, 0 );					
		printC64( 7, 24, "\x9f", skinValues.SKIN_MENU_TEXT_FOOTER, 128, 0 );				
		printC64( 15, 24, "RETURN", skinValues.SKIN_MENU_TEXT_FOOTER, 128, 0 );				
		printC64( 22, 24, "F1", skinValues.SKIN_MENU_TEXT_FOOTER, 128, 0 );					
		ADD_JOY_ITEM( 31, 23, 2, VK_F7 );
		ADD_JOY_ITEM( 7, 24, 1, VK_ESC );
		ADD_JOY_ITEM( 15, 24, 6, VK_RETURN );
		ADD_JOY_ITEM( 22, 24, 2, VK_F1 );
	} else
	if ( subGeoRAM && !subHasKernal )
	{
		...
		printC64( 0, 24, "      ? back, RETURN/F1 launch          ", skinValues.SKIN_MENU_TEXT_FOOTER, 0 );
		//               "012345678901234567890123456789012345XXXX"
		printC64( 27, 23, "F7", skinValues.SKIN_MENU_TEXT_FOOTER, 128, 0 );
		printC64( 6, 24, "\x9f", skinValues.SKIN_MENU_TEXT_FOOTER, 128, 0 );
		printC64( 14, 24, "RETURN", skinValues.SKIN_MENU_TEXT_FOOTER, 128, 0 );
		printC64( 21, 24, "F1", skinValues.SKIN_MENU_TEXT_FOOTER, 128, 0 );
		ADD_JOY_ITEM( 27, 23, 2, VK_F7 );
		ADD_JOY_ITEM( 6, 24, 1, VK_ESC );
		ADD_JOY_ITEM( 14, 24, 6, VK_RETURN );
		ADD_JOY_ITEM( 21, 24, 2, VK_F1 );
	} else
	if ( subSID )
	{
		printC64( 0, 23, "      choose PRG (also via F7)          ", skinValues.SKIN_MENU_TEXT_FOOTER, 0 );
		...
		printC64( 6, 24, "\x9f", skinValues.SKIN_MENU_TEXT_FOOTER, 128, 0 );
		printC64( 14, 24, "RETURN", skinValues.SKIN_MENU_TEXT_FOOTER, 128, 0 );
		printC64( 21, 24, "F3", skinValues.SKIN_MENU_TEXT_FOOTER, 128, 0 );
		ADD_JOY_ITEM( 27, 23, 2, VK_F7 );
		ADD_JOY_ITEM( 6, 24, 1, VK_ESC );
		ADD_JOY_ITEM( 14, 24, 6, VK_RETURN );
		ADD_JOY_ITEM( 21, 24, 2, VK_F1 );
	} else
	{
		...
		//               "012345678901234567890123456789012345XXXX"
		printC64( 4, 23, "F7", skinValues.SKIN_MENU_TEXT_FOOTER, 128, 0 );
		printC64( 16, 23, "F8", skinValues.SKIN_MENU_TEXT_FOOTER, 128, 0 );
		ADD_JOY_ITEM( 4, 23, 2, VK_F7 );
		ADD_JOY_ITEM( 16, 23, 2, VK_F8 );
	}
	...
	if ( modePALNTSC == 1 || modePALNTSC == 2 )
	{
		...
		if ( modeC128 )
		{
			printC64( 36, 24-0*(hasSIDKick?1:0), "8564", skinValues.SKIN_MENU_TEXT_SYSINFO, 0 ); 
		} else
		{
/*			if ( modeVIC == 0 ) // old VIC2
				printC64( 36, 24-0*(hasSIDKick?1:0), "6567", skinValues.SKIN_MENU_TEXT_SYSINFO, 0 ); else
				printC64( 36, 24-0*(hasSIDKick?1:0), "8562", skinValues.SKIN_MENU_TEXT_SYSINFO, 0 ); */
			printC64( 36, 24-0*(hasSIDKick?1:0), "NTSC", skinValues.SKIN_MENU_TEXT_SYSINFO, 0 );
		}
	} else
	if ( modePALNTSC == 3 )
	{
		// Drean
		if ( modeC128 )
			printC64( 36, 24-0*(hasSIDKick?1:0), "8569", skinValues.SKIN_MENU_TEXT_SYSINFO, 0 ); else
			printC64( 36, 24-0*(hasSIDKick?1:0), "6572", skinValues.SKIN_MENU_TEXT_SYSINFO, 0 ); 
	} else
	//if ( modePALNTSC == 4 )
	{
		// PAL
		if ( modeC128 )
		{
			printC64( 36, 24-0*(hasSIDKick?1:0), "8566", skinValues.SKIN_MENU_TEXT_SYSINFO, 0 ); 
		} else
		{
			if ( modeVIC == 0 ) // old VIC2
			{
...
void printSettingsScreen()
{
	...
	if ( !wireSIDAvailable )
		printC64( x+1,  y2+11, "No SID-wire, $D4x0/$D500 won't work!", skinValues.SKIN_ERROR_BAR, 0 );
	...
	if ( settings[2] < 3 )
	{
		...
		extern bool rpiHasAudioJack;
		if ( !rpiHasAudioJack )	settings[ 16 ] = 1;
	...
	} else 
	{
		...
		extern bool rpiHasAudioJack;
		if ( !rpiHasAudioJack )	settings[ 16 ] = 1;
		printC64( x2+10, y2+20, sidStrOutput[ settings[16] ], skinValues.SKIN_MENU_TEXT_ITEM, (l==16)?0x80:0 );
	}
	...
	if ( !wireSIDAvailable )
	{
		unsigned char inactiveColor = 12;

		if ( settings[2] >= 3 )
		{
			for ( int i = 10 * 40; i < 20 * 40; i++ ) c64color[ i ] = inactiveColor;
		} else
		{
			for ( int i = 10 * 40; i < 13 * 40; i++ ) c64color[ i ] = inactiveColor;

			if ( settings[7] < 3 )
				for ( int i = 13 * 40; i < 16 * 40; i++ ) c64color[ i ] = inactiveColor;
		}
	}
...
void renderC64()
{
	...
	if ( menuScreen == MENU_MAIN )
	{
		printMainMenu();
		showLogo = 1;
		c64screenUppercase = 0;
		if ( currentVDCMode == 1 )
			//printC64( 3, 22, "VIC+VDC output, \xde/\xfe for VDC-only", 1, 0 ); 
			printC64( 3, 22, "VIC+VDC, (shift+)\x9e for VDC-only", 1, 0 ); 
		                     
		if ( currentVDCMode == 2 )
			printC64( 4, 22, "VDC output, (shift+)\x9e for VIC", 1, 0 ); 
	} else 
	if ( menuScreen == MENU_BROWSER )
	{
		printBrowserScreen();
		if ( currentVDCMode == 1 )
			printC64( 2, 22, "vic+vdc out, (shift+)\x9e for vdc-only", 1, 0 ); 
		if ( currentVDCMode == 2 )
			printC64( 5, 22, "vdc output, (shift+)\x9e for vic", 1, 0 ); 
		showLogo = 0;
		c64screenUppercase = 1;
	} else
	if ( menuScreen == MENU_CONFIG )
	{
		showLogo = 1;
		...
		c64screenUppercase = 0;
		if ( currentVDCMode == 1 )
			printC64( 4, 22, "VIC+VDC, (shift+)\x9e for VDC-only", 1, 0 ); 
		if ( currentVDCMode == 2 )
			printC64( 5, 22, "VDC output, (shift+)\x9e for VIC", 1, 0 ); 
	} else
