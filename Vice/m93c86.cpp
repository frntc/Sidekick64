...
uint8_t m93c86_data[M93C86_SIZE];
...
void m93c86_write_clock(uint8_t value)
{
    ...
    if ((eeprom_cs == 1) && (value == 1) && (eeprom_clock == 0)) {
        if (command == CMDREADDUMMY) {
            ...
            output_shiftreg = m93c86_data[(m93c86_addr << 1)];

            eeprom_data_out = 0;
            output_count = 0;

            eeprom_data_out = (output_shiftreg >> 7) & 1;
            //LOG(("output %d pos %d m93c86_addr %04x", eeprom_data_out, output_count, m93c86_addr));
            output_shiftreg <<= 1;
            output_count++;

            command = CMDREADDATA;
            //LOG(("load output from %04x with %02x", m93c86_addr, output_shiftreg));
            ...
            switch(output_count) {
                case 8:
                    output_shiftreg = m93c86_data[(m93c86_addr << 1) + 1];
                    //LOG(("reload output from %04x with %02x", m93c86_addr, output_shiftreg));
                    break;
                case 16:
                    m93c86_addr = (m93c86_addr + 1) & ((M93C86_SIZE / 2) - 1);
                    output_shiftreg = m93c86_data[(m93c86_addr << 1)];
                    output_count = 0;
                    //LOG(("reload output from %04x with %02x", m93c86_addr, output_shiftreg));
                    break;
                ...
                case 13:
                    switch (command) {
                        case CMDREAD:
                            ...
                            m93c86_addr = ((input_shiftreg >> 0) & 0x3ff);
                            reset_input_shiftreg();
                            ////LOG(("CMD: read m93c86_addr %04x", m93c86_addr));
                            break;
                        case CMDWDS:
                        ...
                        case CMDERASE:
                            if (write_enable_status == 0) {
                                ...
                                reset_input_shiftreg();
                                m93c86_data[(m93c86_addr << 1)] = 0xff;
                                m93c86_data[(m93c86_addr << 1) + 1] = 0xff;
                                //LOG(("CMD: erase m93c86_addr %04x", m93c86_addr));
                            }
                            break;
                        case CMDERAL:
                            if (write_enable_status == 0) {
                    ...
                    switch (command) {
                        case CMDWRITE:
                            if (write_enable_status == 0) {
                                ...
                                m93c86_addr = ((input_shiftreg >> 16) & 0x3ff);
                                data0 = ((input_shiftreg >> 8) & 0xff);
                                data1 = ((input_shiftreg >> 0) & 0xff);
                                ready_busy_status = STATUSBUSY;
                                reset_input_shiftreg();
                                m93c86_data[(m93c86_addr << 1)] = data0;
                                m93c86_data[(m93c86_addr << 1) + 1] = data1;
                                //LOG(("CMD: write m93c86_addr %04x %02x %02x", m93c86_addr, data0, data1));
                            }
                            break;
                        case CMDWRAL:
                            if (write_enable_status == 0) {
                                ...
                                ready_busy_status = STATUSBUSY;
                                reset_input_shiftreg();
                                for (m93c86_addr = 0; m93c86_addr < (M93C86_SIZE / 2); m93c86_addr++) {
                                    m93c86_data[(m93c86_addr << 1)] = data0;
                                    m93c86_data[(m93c86_addr << 1) + 1] = data1;
                                }
