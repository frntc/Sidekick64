...
SidTune::LoadStatus SidTune::PSID_fileSupport( unsigned char *dataBuf, int dataLen )
{
    int clock, compatibility;
    uint_least32_t speed;
    uint_least32_t bufLen = dataLen;
    ...
    const psidHeader* pHeader = reinterpret_cast<const psidHeader*>(dataBuf);
