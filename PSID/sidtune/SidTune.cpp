...
static const char* savedString = "--- SAVED WITH SIDPLAY ---";
...
bool SidTune::load( unsigned char* sidData, int sidLength, const bool separatorIsSlash)
{
//    cleanup();
    init();
    isSlashedFileName = separatorIsSlash;
    getFromRaw(sidData, sidLength);
    return status;
}
...
void SidTune::init()
{
    ...
    {
        info.commentString[0] = new char[ 40 ];
        strcpy( info.commentString[0], savedString );//SidTuneTools::myStrDup("--- SAVED WITH SIDPLAY ---");
    }
    else
...
/*    if ( dataFileName != 0 )
    ...
    else*/
    {
        ...
        info.path = strdup(emptyString);//::myStrDup("");
        info.dataFileName = strdup(emptyString);//SidTuneTools::myStrDup("");
    }
...
/*    if ( infoFileName != 0 )
    ...
    else*/
    {
        ...
        char emptyString[1] = {0};
        info.infoFileName = strdup(emptyString);//SidTuneTools::myStrDup("");
    }
...
void SidTune::getFromFiles( const char* fileName )
{
	...
	unsigned char *b = new unsigned char[65536];
	int s = 0;
	...
	Buffer_sidtt<const uint_least8_t> fileBuf1( b, s );
...
void SidTune::getFromRaw( unsigned char* sidData, int sidLength )
{
	// Assume a failure, so we can simply return.
	status = false;
	...
	uint_least8_t *bufCopy = new uint_least8_t[65536];
	memcpy( bufCopy, sidData, sidLength );
	Buffer_sidtt<const uint_least8_t> fileBuf1( (const uint_least8_t*)bufCopy, sidLength );
	...
	if ( true )
	{
		...
		// File loaded. Now check if it is in a valid single-file-format.
		ret = PSID_fileSupport( sidData, sidLength );
		if ( ret != LOAD_NOT_MINE )
		{
			if ( ret == LOAD_OK )
				status = acceptSidTune( "dummyfileName", 0, fileBuf1 );
			else
				info.statusString = info.formatString;
			return;
		}
