...
int readConfig( CLogger *logger, char *DRIVE, char *FILENAME )
{
	...
	while ( *cfgPos != 0 )
	{
		if ( getNextLine() && curLine[ 0 ] )
		{
			...
			if ( ptr )
			{
				...
				if ( strcmp( ptr, "SDCARD_DEVICE_NUMBER" ) == 0 && ( ptr = strtok_r( NULL, "\"", &rest ) ) )
				{
					cfgDeviceNumber = atoi( ptr );
					if ( cfgDeviceNumber < 8 ) cfgDeviceNumber = 8;
					if ( cfgDeviceNumber > 15 ) cfgDeviceNumber = 15;
				}
				...
				if ( strcmp( ptr, "FONT_WIDTH" ) == 0 )
				{
					ptr = strtok_r( NULL, " \t", &rest );
					if ( strstr( ptr, "NARROW" ) ) cfgFontWidth = 5;
					if ( strstr( ptr, "WIDE" ) ) cfgFontWidth = 6;
				}
	...
	WAIT_CYCLE_MULTIPLEXER = timingValues[ 3 ];
